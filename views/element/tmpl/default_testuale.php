...
<div class=" col-xs-3">
    <div class="box_info_contenuto col-xs-12 " >
        ...
        <h3> <?php if (!empty($this->allegati)) echo 'Allegati'; ?></h3>
            <form class="microformparametri" action="<?php echo JURI::root(); ?>component/gglms/search" method="POST">
                <ul>
                <?php
                    foreach ($this->allegati as $item) { 
                        $filepath = $this->basefilepath.$item['idfile'].'/'.$item['filename'];
                        FB::log($filepath," filepath ");
                ?>
                <li><a href="<?php echo $filepath ?>"><?php echo $item['nome']; ?> </a></li>
               <?php
                }
                ?> 
                </ul>
