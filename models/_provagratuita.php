 ...
 * @subpackage WebTV
 */
class webtvModelProvagratuita extends JModel {
    ...
    public function check_Coupon() {
        try {
            $query = '
                SELECT
                    *, 
                    DATE_FORMAT( data_scadenza , "%d-%m-%Y" ) as data_scadenza_f,
                    DATE_FORMAT( data_scadenza , "%H:%m" ) as ora_scadenza_f
                FROM 
                    #__gg_coupon as c
                WHERE 
                    c.coupon = "provagratuita'.$this->_userid.'"
                    and 
                    id_utente= '.$this->_userid.'
                ';

            $this->_db->setQuery($query);
            if (false === ($results = $this->_db->loadAssoc()))
                throw new RuntimeException($this->_db->getErrorMsg(), E_USER_ERROR);
            $coupon = empty($results) ? array() : $results;
